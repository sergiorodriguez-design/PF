...
@app.route("/convertir_lote", methods=["OPTIONS"])
def _preflight():
    return "", 204

CONVERSOR = Path(__file__).parent / "conversor_papel_generico_arreglado.py"
...
def convertir_lote():
    ...
    try:
        ...
        incluir_unidades = data.get("incluirUnidades", True)
        archivos         = data.get("archivos", {})
        plantilla        = archivos.get("plantilla")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Base64 de cada unidad por separado (omitido si el cliente lo omite para reducir payload)
